...
export async function test({ token, id, n, mode, networks, duration }) {
	...
	if (mode === "api") {
		...
		writeFileSync(
			...
			`JVM_ARGS="-Xms8192m -Xmx8192m" jmeter -n -t logging/${token}/test-result/test.jmx -l logging/${token}/test-result/result.jtl -j logging/${token}/test-result/jmeter.log`
		);
